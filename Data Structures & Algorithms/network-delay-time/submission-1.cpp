...
public:
    int networkDelayTime(vector<vector<int>>& times, int n, int k) {
        ...
        dist[k] = 0;
        pq.push({0, k}); // Push the starting point into our min-heap also the min is decided based on first element then 2nd element
        
        // ---------------------------------------------------------
        // STEP 3: The Dijkstra Loop (Exploring the network)
        // ---------------------------------------------------------
        while (!pq.empty()) {
            //when min heap is not empty we look at the element at the top
            // Pull out the node that currently has the smallest accumulated time with the node time and id and then pop them out
            ...
            if (current_time > dist[current_node]) {
                continue;
            }//if the current time we got for this particular ticket of the node is larger than the dist[current node] then we throw that and go to the next min heap ticket.
            //priority queue holds multiple entries for the same node   
            // Look at all neighboring nodes connected to our current node
            for (int i = 0; i < graph[current_node].size(); i++) {//graph u will have all the pairs it is connected to
                int neighbor_node = graph[current_node][i].first;//consider all the neighbours and their travel time.
                ...
                int total_time = current_time + travel_time;//add it to the current time to reach till the current node
                
                // If this new path is faster than any previously recorded path to this neighbor...
                if (total_time < dist[neighbor_node]) {//check if the time we got is less than the time it already calcuulated as lowest if yes then change it.
