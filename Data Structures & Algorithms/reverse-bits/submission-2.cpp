...
public:
    uint32_t reverseBits(uint32_t n) {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            result <<= 1;//remove the digit on the left most and add a zero at the riht most
            
            // 2. Extract the rightmost bit of 'n' (using n & 1) 
            // and slap it onto our result using bitwise OR (|)
            result |= (n & 1);//extract the last bit of n and then add it to result.
            
            // 3. Shift 'n' to the right to move on to the next bit
            n >>= 1;//remove the right most digit and add a 0 at the left most
        }
